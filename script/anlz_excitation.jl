...
get_runid_IB_runid(runid_IB) = first(runid_IB)
get_runid_IB_IB(runid_IB) = last(runid_IB)
get_runid_IB_date(runinfo, idx_runid_IB) = hasproperty(runinfo, :date_runid) ? first(runinfo.date_runid[idx_runid_IB]) : runinfo.date
get_runid_IB_file_runid(runinfo, idx_runid_IB, runid_IB) = hasproperty(runinfo, :date_runid) ? last(runinfo.date_runid[idx_runid_IB]) : get_runid_IB_runid(runid_IB)
get_runid_IB_runinfo(runinfo, idx_runid_IB, runid_IB) = merge(
    runinfo,
    (;
        date=get_runid_IB_date(runinfo, idx_runid_IB),
        runid=get_runid_IB_file_runid(runinfo, idx_runid_IB, runid_IB),
        IB=get_runid_IB_IB(runid_IB),
    ),
)
...
if hasproperty(runinfo, :date_runid)
    runids_io = last.(runinfo.date_runid)
    runids_axis = get_runid_IB_runid.(val_vars.runid_IB)
...
info_fmt = [
    Dict(
        ...
        "runid" => get_runid_IB_runid(val_vars.runid_IB[c]),
        "ib" => get_runid_IB_IB(val_vars.runid_IB[c]),
    )
...
essn_stacked_over_rep = [
    begin
        print("\r  [$tag] stacking over rep runid_IB_idx=$c t_idx=$t istp_idx=$i")
        flush(stdout)
...
essn_stacked_over_rep_t = [
    begin
        ...
        print("\r  [$tag] stacking over rep+t runid_IB_idx=$c istp_idx=$i n=$(length(essns_rt))")
        flush(stdout)
...
extr_fmt = [
    begin
        if r == first(axes(essn_2d_fmt, 2)) && (t == first(axes(essn_2d_fmt, 3)) || t % 25 == 0 || t == last(axes(essn_2d_fmt, 3)))
            print("\r  [$tag] extracting shots runid_IB_idx=$c t_idx=$t istp_idx=$i")
            flush(stdout)
        end
...
extr_stacked_over_rep = [
    begin
        if t == first(axes(essn_stacked_over_rep, 2)) || t % 25 == 0 || t == last(axes(essn_stacked_over_rep, 2))
            print("\r  [$tag] extracting stacked runid_IB_idx=$c t_idx=$t istp_idx=$i")
            flush(stdout)
        end
...
modes_pca_modl2d = [
    begin
        println("  [$tag] fitting PCA runid_IB_idx=$c istp_idx=$i")
        flush(stdout)
...
## Overall plots
for (c, runid_IB) in enumerate(val_vars.runid_IB)
    tag_runid_IB = gen_run_tag(get_runid_IB_runinfo(runinfo, c, runid_IB))
    runinfo_plot = get_runid_IB_runinfo(runinfo, c, runid_IB)

    t_stage = log_step("building trend figures for $tag_runid_IB")
    fig_trend, axs_trend = set_axis_sidepeak_nvlp!(n_dim_vars_per_runid_IB, set_panel_trend_sidepeak_nvlp!, runinfo_plot)
    fig_nvlp, axs_nvlp = set_axis_stack_all!(n_dim_vars_per_runid_IB, set_panel_trend_nvlp!, runinfo_plot)
    log_done("built trend figures for $tag_runid_IB", t_stage)
    for i in 1:n_istp
        t_plot_stage = log_step("plotting and saving trends for $tag_runid_IB istp=$(val_vars.istp[i])")
        ...
        for format in ["pdf", "png"]
            fig_trend |> f -> save(joinpath(path_output, @sprintf("%s_%s_trend.%s", tag_runid_IB, val_istp, format)), f; backend=CairoMakie)
            fig_nvlp |> f -> save(joinpath(path_output, @sprintf("%s_%s_trend_nvlp.%s", tag_runid_IB, val_istp, format)), f; backend=CairoMakie)
        end
        log_done("saved trends for $tag_runid_IB istp=$(val_vars.istp[i])", t_plot_stage)
    end

    t_stage = log_step("building and saving PCA figure for $tag_runid_IB")
    ...
    fig_pca |> f -> save(joinpath(path_output, @sprintf("%s_pca.pdf", tag_runid_IB)), f; backend=CairoMakie)
    log_done("saved PCA figure for $tag_runid_IB", t_stage)
end
# fig_trend |> display
##
...
# fig_full, axs_solo, axs_stacked = set_axis_full(n_dim_vars_per_runid_IB, set_panel_solo_essn_2d!)
# fig_full, axs_solo, axs_stacked = set_axis_full(n_dim_vars_per_runid_IB, set_panel_solo_modl!)
# for c in 1:n_dim_vars[1], r in 1:n_dim_vars[2], t in 1:n_dim_vars[3], i in 1:n_dim_vars[4]
...
# end
# println("Full axes ready: dimensions $(n_dim_vars_per_runid_IB)")
