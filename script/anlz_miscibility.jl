...
xy_peak_duet = r.dens_full_fmt |>
    ...
    ds;
    dims=ndims(ds),
) |> p -> repeat(p, inner=ntuple(i -> i == idx_istp_axis ? n_istp_per_condition : 1, length(r.n_dim_vars)))
...
info_fmt = [
    Dict("istp" => r.val.istp[i], "t_hold" => r.val.t_hold[t], "repeat" => r.val.rep[rep], "ib" => r.val.IB[c], "bias" => r.val.bias[b])
...
for (c, ib) in enumerate(r.val.IB), (b, bias) in enumerate(r.val.bias)
    ax = axs_sizes[c, b]["ax"]
    [ax] |> clear_axes!
    for (i, istp_iter) in enumerate(r.val.istp), rep in r.val.rep
...
for (c, ib) in enumerate(r.val.IB), (t, t_hold_iter) in enumerate(r.val.t_hold)
    ax = axs_sizes[c, t]["ax"]
    [ax] |> clear_axes!
    for (i, istp_iter) in enumerate(r.val.istp), rep in r.val.rep
        clr_theme = Oklch(0.52, 0.14, hue_theme_istp[istp_iter])
...
#     fig_stacked_duets, axs_stacked_duets = set_axes_v_t_rep!(Base.setindex(r.n_dim_vars, 1, 2)[2:end], set_panel_misc_duet_2d!, r.runinfo, info_stacked_over_rep[c, :, :, :, :]; partidx=c)
...
# for c in 1:r.n_dim_vars[1]
#     fig_full_duets, axs_full_duets = set_axes_v_t_rep!(Tuple(r.n_dim_vars)[2:end], set_panel_misc_duet_2d!, r.runinfo, info_fmt[c, :, :, :, :]; partidx=c)
