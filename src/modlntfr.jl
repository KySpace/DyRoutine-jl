...
function calc_centered_cross_profile(
    ...
    x0, y0, A_narrow, sigma_x_narrow, sigma_y_narrow, A_wide, sigma_wide = fit_density.params[1:7]
    beta = length(fit_density.params) >= 8 ? fit_density.params[8] : 0.0
    ...
    sigma_narrow = axis == :column ? sigma_y_narrow : sigma_x_narrow
    tail_raw = @. A_wide * exp(-s_profile^2 / (2 * sigma_wide^2))
    narrow_raw = @. A_narrow * exp(-s_profile^2 / (2 * sigma_narrow^2))
    tail = tail_raw
