...
function double_gaussian_disk_2d_model_abrr(coords, params)
    length(params) == 11 || throw(ArgumentError(
        ...
        "(x0, y0, A_narrow, sigma_x_narrow, sigma_y_narrow, A_wide, sigma_wide, beta, skew_x, skew_y, θ), got $(length(params)).",
    ))
    x0, y0, A_narrow, sigma_x_narrow, sigma_y_narrow, A_wide, sigma_wide, beta, skew_x, skew_y, θ = params
    x = @view coords[1, :]
    y = @view coords[2, :]
    dx = @. x - x0
    dy = @. y - y0
...
function expand_fit_params_abrr(params)
    length(params) == 11 || throw(ArgumentError(
        ...
        "(x0, y0, A_narrow, sigma_x_narrow, delta_sigma_y, A_wide, sigma_wide, beta, skew_x, skew_y, θ), got $(length(params)).",
    ))
    x0, y0, A_narrow, sigma_x_narrow, delta_sigma_y, A_wide, sigma_wide, beta, skew_x, skew_y, θ = params
...
function fit_two_gaussian_2d(
    ...
    p_lower = [-center_bound, -center_bound, eps(Float64), step_x, 0.0, eps(Float64), sigma_wide_min, 0.0, 0.0, -10.0, -π / 2]
    p_upper = [center_bound, center_bound, Inf, r_narrow_max, r_narrow_max, Inf, max_x * 3, 2.0, 0.0, 10.0, π / 2]
    fit = curve_fit(
...
function calc_centered_cross_profile(
    ...
    x0, y0, A_narrow, sigma_x_narrow, sigma_y_narrow, A_wide, sigma_wide, beta, skew_x, skew_y, θ = fit_density.params
